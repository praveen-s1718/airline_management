...
const {Admin,Flight,Airport,Booking,Passenger,Airline, Itinerary} = require('../backend/airline.model.js');
...
const searchMultiCityFlights = async (req, res) => {
    try {
        const { segments } = req.body; // Array of { from, to, date }
        ...
        res.status(500).json({ error: 'Error finding multi-city flights' });
    }
};
...
const bookItinerary = async (req, res) => {
    ...
    try {
        const { username, flightsToBook } = req.body; // flightsToBook array of { flightId, seat, seatClass, price }
        
        const passenger = await Passenger.findOne({ username }).session(session);
        if (!passenger) {
            throw new Error('Passenger not found');
        }

        const itineraryId = 'PNR' + Math.random().toString(36).substring(2, 8).toUpperCase();
        const bookedFlightIds = [];
        let totalPrice = 0;

        for (const f of flightsToBook) {
            const flight = await Flight.findOne({ flightNumber: f.flightId }).session(session);
            if (!flight) throw new Error(`Flight ${f.flightId} not found`);
            if (flight.departure.scheduledTime <= new Date()) {
                throw new Error(`Flight ${f.flightId} has already departed`);
            }
            if (!flight.seatAvailability[f.seatClass.toLowerCase()].includes(f.seat)) {
                throw new Error(`Seat ${f.seat} not available on flight ${f.flightId}`);
            }
            ...
            bookedFlightIds.push(booking._id);
            totalPrice += Number(f.price);

            // Update flight
            flight.bookings.push({ bookingId: booking._id, email: passenger.email });
            flight.seatAvailability[f.seatClass.toLowerCase()] = flight.seatAvailability[f.seatClass.toLowerCase()].filter(s => s !== f.seat);
            await flight.save({ session });

            // Update passenger
            passenger.bookings.push({ bookingId: booking._id, bookingStatus: 'confirmed' });
        }
        ...
        try {
            const mailOptions = {
                from: 'airlinemanagment1234@gmail.com',
                to: passenger.email,
                subject: `Itinerary Confirmation - ${itineraryId}`,
                text: `Dear ${passenger.name},\n\nYour multi-city trip (PNR: ${itineraryId}) has been confirmed. Total Price: ${totalPrice}\n\nThank you!`
            };
            await transporter.sendMail(mailOptions);
