...
const Confirmation = () => {
  ...
  const handleClick = async (event) => {
    ...
    if(info.flightNumber){
      try {
        const response = await fetch(`https://airline-management-tco0.onrender.com/api/bookings?flightId=${info.flightNumber}&username=${username}&seat=${seatSelected}&seatClass=economy&price=${info.price.economy}`, {
          method: "POST",
          headers: {
