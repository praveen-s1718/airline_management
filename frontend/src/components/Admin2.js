...
const Admin2 = () => {
    ...
    const fetchAirportsByCountry = async (countryName) => {
        ...
        setError(null);
        try {
            const response = await fetch(`https://airline-management-tco0.onrender.com/api/airports/country/${countryName}`);
            if (!response.ok) {
    ...
    const handleDeleteAirport = async (airportCity) => {
        try {
            const response = await fetch(`https://airline-management-tco0.onrender.com/api/airports/${airportCity}`, {
                method: 'DELETE'
            });
